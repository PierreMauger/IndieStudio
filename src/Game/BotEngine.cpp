...
void BotEngine::recursive(GameScene *gameScene, glm::vec3 pos, const int &bot_key)
{
    ...
    while (neighbor && !this->_found) {
        switch (neighbor) {
            case 1: this->recursive(gameScene, glm::vec3(pos.x + 1.f, pos.y, pos.z), bot_key);
                break;
            case 2: this->recursive(gameScene, glm::vec3(pos.x, pos.y + 1.f, pos.z), bot_key);
                break;
            case 3: this->recursive(gameScene, glm::vec3(pos.x - 1.f, pos.y, pos.z), bot_key);
                break;
            case 4: this->recursive(gameScene, glm::vec3(pos.x, pos.y - 1.f, pos.z), bot_key);
                break;
            default:
                break;
        }
